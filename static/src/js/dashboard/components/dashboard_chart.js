...
export class DashboardChart extends Component {
  ...
  initializeChart() {
    ...
    const chartData = {
      ...
      datasets: [
        {
          label: block.name, // atau block.config?.group_by
          data: block.data.datasets?.[0]?.data || [],
          backgroundColor: this.getChartColors(block.data.labels?.length || 0),
          borderColor:
            block.config.chart_type === "line" ||
            block.config.chart_type === "radar"
              ? this.getChartColors(1, true)
              : undefined,
          borderWidth: 2,
          fill: block.config.chart_type === "radar",
        },
      ],
    };
    ...
    const config = {
      ...
      options: {
        ...
        scales:
          block.config.chart_type !== "pie" &&
          block.config.chart_type !== "doughnut"
            ? {
                y: {
                  beginAtZero: true,
                  ticks: {
  ...
  async handleChartClick(label) {
    const block = this.props.block;
    const groupByField = block.config?.group_by;
    if (!groupByField) {
      this.notification.add(
        "Cannot open records: No group_by field configured",
        { type: "warning" },
      );
      return;
    }

    let domain = [[groupByField, "=", label]];
    const modelName = block.model_name;

    // --- Deteksi jika field adalah many2one (perlu ID, bukan nama) ---
    try {
      const fieldInfo = await this.orm.call(
        "dashboard.block",
        "get_field_info",
        [block.id, groupByField],
      );
      if (fieldInfo && fieldInfo.type === "many2one") {
        // Cari record dengan display name = label
        const records = await this.orm.searchRead(
          fieldInfo.model,
          [[fieldInfo.display_field || "name", "=", label]],
          ["id"],
        );
        if (records && records.length > 0) {
          domain = [[groupByField, "=", records[0].id]];
        } else {
          // Jika label berupa angka (ID) langsung pakai
          if (isNaN(label)) {
            this.notification.add(`No record found for "${label}"`, {
              type: "warning",
            });
            return;
          }
      ...
      console.warn("Field info error, using label as-is", err);
    }

    // Buka list view dengan domain filter
    try {
      await this.action.doAction({
        type: "ir.actions.act_window",
        res_model: modelName,
